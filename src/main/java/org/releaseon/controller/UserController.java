...
import static org.releaseon.utils.response.ResponseCode.USER_INVALID_ACCOUNT;
...
public class UserController {
    ...
    private BaseResponse login(HttpServletRequest request, String username, String password) {
        ...
        try {
            ...
            return ResponseUtil.fail(USER_INVALID_ACCOUNT, uae.getMessage());
        } catch (LockedAccountException lae) {
            return ResponseUtil.fail(USER_INVALID_ACCOUNT, lae.getMessage());
        } catch (AuthenticationException ae) {
            return ResponseUtil.fail(USER_INVALID_ACCOUNT, "用户名或密码错误");
        }
    ...
    public BaseResponse register(@RequestBody User user, HttpServletRequest request) {
        ...
        return ResponseUtil.ok("注册成功，等待管理员启用");
    }
