/**
 * CSRF 保护：自动为所有 AJAX 请求添加 CSRF token 头
 */
$.ajaxSetup({
    headers: {
        'X-XSRF-TOKEN': typeof csrfToken !== 'undefined' ? csrfToken : ''
    }
});
// Toast 默认设置
$.toast.options.heading = '';
$.toast.options.position = 'bottom-center';
$.toast.options.hideAfter = 3000;
...
function bindActions() {
    ...
    $(".app-delete").click(function () {
        ...
        var self = $("." + li);
        var version = $(this).closest("li").find("b").text().trim() || "此版本";
        confirmAction("确定要删除「" + version + "」吗？删除后不可恢复。", function () {
            $.post(url, function (result) {
...
$(function () {
    ...
    $("#delete-app").click(function () {
        ...
        var name = $("#appName").text().trim() || "此应用";
        confirmAction("确定要删除「" + name + "」吗？删除后不可恢复。", function () {
            $.post(url, function (result) {
